...
def train_one_epoch(model, otpimizer, scheduler, loss_fn, train_loader, val_loader, device, epoch_idx):
    ...
    val_f_score = 0
    with torch.no_grad:
        vpbar = tqdm(enumerate(train_loader), total=len(train_loader), desc=f"validation: {epoch_idx}", unit="image")
        ...
        vpbar.set_postfix(**{'val_loss': val_loss,
...
if __name__ == '__main__':
    ...
    # 加入余弦退火算法
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, eta_min=1e-5)
